...
func setTokenURI{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(
    ...
    // OnlyOwner in solidity, only the owner of the contract can call this setter 
...
func mint{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}() {
    // We get the address of the function caller
    let (caller_address) = get_caller_address();
    // We get the next_token id(storage_var)
    let (tokenId: Uint256) = next_token.read();
    // Mint the token => caller
    ERC721._mint(caller_address, tokenId);
    let (next_tokenId, _) = uint256_add(tokenId, Uint256(1, 0));
    // Increment the next_tokenId variable
